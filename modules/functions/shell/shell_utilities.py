...
class TimestampUtility:
    def __init__(self):
        now = datetime.now()
        self.YYYY = now.strftime("%Y")
        self.MM = now.strftime("%m")
        self.DD = now.strftime("%d")
        self.hh = now.strftime("%H")
        self.mm = now.strftime("%M")
        self.ss = now.strftime("%S")
        self.timestamp_full = now.strftime("%Y_%m_%d-%H_%M_%S")
        self.timestamp_date = now.strftime("%Y_%m_%d")
        self.timestamp_time = now.strftime("%H_%M_%S")

    def get_timestamps(self):
        """
        Return a dictionary of timestamps.
        """
        return {
            "YYYY": self.YYYY,
            "MM": self.MM,
            ...
            "mm": self.mm,
            "ss": self.ss,
            "timestamp_full": self.timestamp_full,
...
if __name__ == "__main__":
    timestamps = TimestampUtility().get_timestamps()
    for key, value in timestamps.items():
        print(f"{key}: {value}")
    print(f"Static Timestamp Full: {TimestampUtility.timestamp_full()}")
    print(f"Static Timestamp Date: {TimestampUtility.timestamp_date()}")
    print(f"Static Timestamp Time: {TimestampUtility.timestamp_time()}")
