...
# File Name:        pyside6_qt_clickable_line_edit.py
# Version:          1.0.3
# Created:          2024-01-19
# Modified:         2025-02-25
...
class pyside6_qt_clickable_line_edit(QtWidgets.QLineEdit):
    '''
    Custom Qt Flame Clickable Line Edit Widget

    pyside6_qt_clickable_line_edit(text, connect[, width=150, max_width=2000])
    ...
    Example:
        ...
        clickable_lineedit =  pyside6_qt_clickable_line_edit('Some Text', some_function)
    '''

    clicked = QtCore.Signal()

    def __init__(self, text: str, connect: Callable[..., None], width: Optional[int]=150, max_width: Optional[int]=2000):
        super(pyside6_qt_clickable_line_edit, self).__init__()

        # Check argument types

        if not isinstance(text, str):
            raise TypeError('pyside6_qt_clickable_line_edit: text must be a string')
        elif not isinstance(width, int):
            raise TypeError('pyside6_qt_clickable_line_edit: width must be an integer')
        elif not isinstance(max_width, int):
            raise TypeError('pyside6_qt_clickable_line_edit: max_width must be an integer')
