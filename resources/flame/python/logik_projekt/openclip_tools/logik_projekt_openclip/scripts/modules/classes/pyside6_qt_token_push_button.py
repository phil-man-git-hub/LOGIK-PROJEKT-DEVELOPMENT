...
# File Name:        pyside6_qt_token_push_button.py
# Version:          1.0.2
# Created:          2024-01-19
# Modified:         2025-01-19
...
class pyside6_qt_token_push_button(QtWidgets.QPushButton):
    '''
    Custom Qt Flame Token Push Button Widget

    pyside6_qt_token_push_button(button_name, token_dict, token_dest[, button_width=150, button_max_width=300])
    ...
    Example:
        ...
        token_push_button = pyside6_qt_token_push_button('Add Token', token_dict, token_dest)
    '''
    ...
    def __init__(self, button_name: str, token_dict: Dict[str, str], token_dest, button_width: Optional[int]=150, button_max_width:Optional[int]=300):
        super(pyside6_qt_token_push_button, self).__init__()
        from functools import partial

        # Check argument types

        if not isinstance(button_name, str):
            raise TypeError('pyside6_qt_token_push_button: button_name must be a string.')
        if not isinstance(token_dict, dict):
            raise TypeError('pyside6_qt_token_push_button: token_dict must be a dictionary.')
        if not isinstance(token_dest, QtWidgets.QLineEdit):
            raise TypeError('pyside6_qt_token_push_button: token_dest must be a QLineEdit.')
        if not isinstance(button_width, int):
            raise TypeError('pyside6_qt_token_push_button: button_width must be an integer.')
        if not isinstance(button_max_width, int):
            raise TypeError('pyside6_qt_token_push_button: button_max_width must be an integer.')
