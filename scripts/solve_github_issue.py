    ...
    for subfile in SUBFILES:
        ...
        if not os.path.exists(subfile_path):
            with open(subfile_path, "w") as f:
                f.write(STARTER_CONTENT[subfile].format(issue_number))

    # Append how-to section with sub-steps to the to-do document
    todo_path = os.path.join(issue_dir, "to-do", "to-do.md")
    howto_path = os.path.join(issue_dir, "how-to", "how-to.md")
    if os.path.exists(todo_path) and os.path.exists(howto_path):
        with open(howto_path, "r") as f:
            howto_content = f.read()
        # Extract step-by-step section from how-to
        steps = []
        in_steps = False
        for line in howto_content.splitlines():
            if line.strip().startswith("## Step-by-Step Implementation"):
                in_steps = True
                continue
            if in_steps:
                if line.strip().startswith("##") and not line.strip().startswith("## Step-by-Step Implementation"):
                    break
                steps.append(line)
        steps_text = "\n".join(steps).strip()
        with open(todo_path, "a") as f:
            ...
            f.write(steps_text + "\n")
    print(
        f"Created starter .md subfiles in subdirectories for issue {issue_number} in {issue_dir} and updated to-do document."
    )
