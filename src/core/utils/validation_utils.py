...
def validate_init_config(init_config_name: str, resolution_w: str, resolution_h: str, frame_rate: str) -> tuple[bool, str]:
    ...
    # Extract numeric part from init_config_name
    # (e.g., "1920x1080@25000p.cfg" -> "25000")
    init_config_framerate_match = re.search(
        r'@(\d+)(p|psf)?\.cfg', init_config_name
    )
    init_config_framerate_num = ""
    if init_config_framerate_match:
        init_config_framerate_num = init_config_framerate_match.group(1)

    # Extract numeric part from selected frame_rate
    # (e.g., "25.000 fps" -> "25000")
    selected_frame_rate_num = (
        frame_rate.replace(".", "")
        .replace(" fps", "")
        .strip()
    )
    ...
    if mismatch_found:
        message = (
            "Your " + " and / or ".join(warning_message_parts) +
            " do not match your Init Config. Continue anyway?"
        )
