...
def find_repo_dir():
    ...
    bin_dir = this_script_dir
    if '/bin/' in this_script_dir:
        bin_dir = this_script_dir.split('/bin/')[0]
    return os.path.dirname(bin_dir) if os.path.dirname(bin_dir) else this_script_dir
...
def main():
    ...
    # Load logik-projekt-site-prefs.json
    ...
    # Determine output paths based on chosen_folder_basename
    ...
    # Update or add the entry in PROJEKT Configurations
    found_entry = False
    ...
    # Get output paths from the selected configuration (now potentially updated)
