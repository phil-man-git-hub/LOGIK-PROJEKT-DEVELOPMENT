...
module without user interaction. This API has the same functionalities as the
export in Flame.

This module contains examples of custom actions that use the Python
export API (PyExporter) and do a post export actions with three different methods
(command line / Python / natively) in foreground and background.
"""
...
def export_clip_and_zip(selection, foreground):
    """
    Export all clips in a selection and zip them afterward.

    :param selection: Selection to export
    :param foreground: True to perform the export in the foreground,
                       False to perform the export int he background.
    """
...
def export_clip_and_zip_foreground(selection):
    """
    Export all clips in a selection as QuickTime, and then zip them.
    Performed in the foreground.

    :param selection: Selection to export
    """
    return export_clip_and_zip(selection, foreground=True)


def export_clip_and_zip_background(selection):
    """
    Export all clips in a selection as QuickTime, and then zip them.
    ...
    :param selection: Selection to export
    """
    return export_clip_and_zip(selection, foreground=False)
...
# Python code is shared, the functionality that can be done will be the same.
# It is good to note that the Python interpreter that will run the post export
# callback differs from the one sending the background job.
#
...
# the command line arguments (or a file passed through the arguments) to be used
# in the post export hook callback.
################################################################################


def create_python_backburner_job(
    job_name, description, dependencies, function, args=None
):
    """
...
def rename_function(old, new):
    ...
    using os.rename to rename a file.
    """
    if "[" in old:
        raise Exception("Unsupported: renaming file sequence pattern")
...
def export_movie_and_rename(selection, foreground):
    """
    Export a movie for each clip in the selection, and then add the suffix
    _renamed.
...
def export_movie_and_rename_foreground(selection):
    """
    Export a movie for each clip in the selection, and then add the suffix
    _renamed. Performed in foreground.
...
def export_movie_and_rename_background(selection):
    """
    Export a movie for each clip in the selection, and then add the suffix
    _renamed. Performed in background.
...
################################################################################
# The following method shows how to chain a ffmpeg job after each exported clip.
################################################################################


def export_clip_and_transcode(selection):
    """
    Export each clip in the selection and transcode with
    ffmpeg to a movie with the suffix _transcoded.
    ...
    class HooksOverride(object):
        def __init__(self):
            self.IMAGE_SEQ_REGEX = re.compile(r"(?P<base>.*)\[(\d+)-(\d+)\](\.\w+)?\Z")
            self.MOVIE_REGEX = re.compile(r"(?P<base>.*?)(\.\w+)?\Z")

        def postExportAsset(self, info, userData, *args, **kwargs):
            del userData, args, kwargs  # Unused necessary parameters
            mio_pattern = os.path.join(info["destinationPath"], info["resolvedPath"])

            if "[" in mio_pattern:
                # Need to replace MIO file pattern "base[a-b].ext" with ffmpeg
                # compatible one, e.g. "base%02d.ext" and add -start_number and
                # -frames options...
                match = self.IMAGE_SEQ_REGEX.match(mio_pattern)
                if not match:
                    raise Exception("Invalid file sequence pattern")
                ...
                in_pattern = (
                    ...
                    + " -i "
                    + match.group("base")
                    # Need to double the % to avoid cmdjob interpretation
                    + r"%%0"
                    + str(len(match.group(2)))
                    + "d"
                    + match.group(4)
                    # -frames is an output option - use it after the -i option
                    # to limit the output to the selected frames range
                    + " -frames "
                    + str(end - start + 1)
                )

            else:
                match = self.MOVIE_REGEX.match(mio_pattern)
                if not match:
                    raise Exception("Invalid movie file pattern")

                in_pattern = "-i " + match.group(0)

            out_pattern = match.group("base") + "_transcoded.mov"

            cmd = "/usr/local/bin/ffmpeg %s -vcodec qtrle %s" % (
                in_pattern,
                out_pattern,
            )

            create_backburner_job(
                job_name="Transcoding %s" % info["assetName"],
                description="Transcoding %s" % info["assetName"],
                dependencies=info["backgroundJobId"],
                cmd=cmd,
            )
...
#
# This is the only method that lets the post export callback trigger an action
# in Flame when sending a background job.
#
...
def export_clip_and_reimport_foreground(selection):
    """
    Export all clips in a selection as QuickTime, and then reimport them in
    Batch. Perfomed in foreground.
    ...
    class HooksOverride(object):
        ...
        def postExportAsset(self, info, userData, *args, **kwargs):
            ...
            #
            full_path = os.path.join(info["destinationPath"], info["resolvedPath"])
            flame.batch.import_clip(full_path, flame.batch.reels[0].name.get_value())

    export_clips(selection, foreground=True, hooks=HooksOverride())


def export_clip_and_reimport_background(selection):
    """
    Export all clips in a selection as QuickTime, and then reimport them in
    Batch. Perfomed in background.
    ...
    jobs_infos = []
    export_clips(
        selection, foreground=False, hooks=HooksOverride(), hooks_user_data=jobs_infos
    )
    ...
    class ReImportCallback(object):
        def __init__(self, jobs_infos):
            ...
            self._backburner_server_handle = WireTapServerHandle(backburner_server_id)
        ...
        def __call__(self):
            ...
            if not node_handle.getMetaData("state", "", 0, state):
                print(
                    ...
                    % (node_handle.getNodeId().id(), node_handle.lastError())
                )
...
def export_clip(clip, export_dir, foreground, hooks, hooks_user_data):
    """
    Export a clip to export_dir
    ...
    examples = [
        {
            "visib": flame.PyExporter.PresetVisibility.Autodesk,
    ...
    preset_dir = flame.PyExporter.get_presets_dir(
        selected_example["visib"], selected_example["type"]
    )
    preset_path = os.path.join(
        preset_dir, selected_example["dir"], selected_example["preset"]
    )
    ...
    #
    exporter.export(
        clip, preset_path, export_dir, hooks=hooks, hooks_user_data=hooks_user_data
    )


def export_clips(selection, foreground, hooks, hooks_user_data=None):
    """
    Export all clips in a selection.
    ...
    for item in selection:
        ...
        if isinstance(item, (flame.PySequence, flame.PyClip)):
            export_clip(item, export_dir, foreground, hooks, hooks_user_data)
    return export_dir
...
def get_export_dir():
    ...
    """
    return os.path.join(
        "/var/tmp/export", datetime.datetime.now().strftime("%Y_%m_%d__%H_%M_%S")
    )
            ...
            "actions": [
                {
                    ...
                    "execute": export_clip_and_zip_foreground,
                },
                {
                    "name": "Export selection and zip (Background)",
                    "execute": export_clip_and_zip_background,
                },
                {
                ...
                },
                {
                    ...
                    "execute": export_clip_and_reimport_foreground,
                },
                {
                    "name": "Export selection and reimport (Background)",
                    "execute": export_clip_and_reimport_background,
                },
                {
                    "name": "Export selection and transcode (Background)",
                    "execute": export_clip_and_transcode,
                },
            ],
        }
    ]
